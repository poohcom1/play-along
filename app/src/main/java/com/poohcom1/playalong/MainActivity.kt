...
fun MainContainer() {
    ...
    Column {
        ...
        Row {
            Button(onClick = {
                setUiState(uiState.copy(popup = PopupType.VIDEO_URL))
            }) {
    ...
    Row {
        Button(onClick = { tempoTapCalculator.tap() }) {
            Text(text = "Tap tempo")
        }
    }
    ...
    when (uiState.popup) {
        PopupType.VIDEO_URL -> {
            VideoUrlPopup(onDismiss = {
                setUiState(uiState.copy(popup = PopupType.NONE))
            }, onUrlSubmit = {
                setUiState(uiState.copy(popup = PopupType.NONE))
                loading = true
